...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['TEST_TEWST_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TEST_TEWST_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Test Tewst API.
 */
export class TestTewst {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['TEST_TEWST_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TEST_TEWST_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.TestTewstError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the TestTewst client with an apiKey option, like new TestTewst({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? TestTewst.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('TEST_TEWST_LOG'), "process.env['TEST_TEWST_LOG']", this) ??
      defaultLogLevel;
  ...
  static TestTewst = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static TestTewstError = Errors.TestTewstError;
...
TestTewst.Pets = Pets;
TestTewst.Store = Store;
TestTewst.UserResource = UserResource;
export declare namespace TestTewst {
